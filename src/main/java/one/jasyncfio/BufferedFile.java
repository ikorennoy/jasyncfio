...
public class BufferedFile extends AbstractFile {
    ...
    BufferedFile(String path, long pathAddress, int fd, PollableStatus pollableStatus, EventExecutor executor) {
        super(path, pathAddress, fd, pollableStatus, executor);
    }

    public static CompletableFuture<BufferedFile> open(Path path, EventExecutor executor, OpenOption... openOptions) {
    ...
    public static CompletableFuture<BufferedFile> open(Path path, int mode, EventExecutor executor, OpenOption... openOptions) {
    ...
    public static CompletableFuture<BufferedFile> open(String path, EventExecutor executor, OpenOption... openOptions) {
        return open(path, 438, executor, openOptions);
    }

    public static CompletableFuture<BufferedFile> open(String path, int mode, EventExecutor executor, OpenOption... openOptions) {
        long pathPtr = MemoryUtils.getStringPtr(path);
        return executor.executeCommand(
                Command.openAt(
                        OpenOption.toFlags(openOptions),
                        pathPtr,
                        mode,
                        executor,
                        CompletableFutureResultProvider.newInstance()
                )
        ).thenApply((fd) -> new BufferedFile(path, pathPtr, fd, PollableStatus.NON_POLLABLE, executor));
    }
}
